...
from torchvision.models.detection.roi_heads import RoIHeads, fastrcnn_loss, keypointrcnn_inference, \
    keypointrcnn_loss, maskrcnn_inference
...
def focal_loss(class_logits, labels, alpha=0.25, gamma=2.0):
    ...
    loss = -alpha * focal_weight * torch.log(target_probs)
...
def focal_fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    """
    Computes the focal loss for Faster R-CNN.
...
def project_masks_on_boxes_with_dif_side(gt_masks, boxes, matched_idxs, M, N):
    ...
    rois = torch.cat([matched_idxs[:, None], boxes], dim=1)
    gt_masks = gt_masks[:, None].to(rois)
    return roi_align(gt_masks, rois, (M, N), 1.0)[:, 0]
...
def resize_attention_masks(attention_masks, target_size):
    """
    Resize each tensor in the list of attention masks to the specified target size.
    """
    resized_attention_masks = []
    for mask in attention_masks:
        if mask.dtype != torch.float32:
            mask = mask.float()
        resized_mask = F.interpolate(mask.unsqueeze(1), size=target_size, mode='bilinear', align_corners=False)
        resized_mask = (resized_mask > 0.5).float()
        resized_attention_masks.append(resized_mask.squeeze(1))
    return resized_attention_masks


def normalize_rows(tensor):
    row_max = tensor.max(dim=2, keepdim=True)[0]

    row_max[row_max == 0] = 1

    normalized_tensor = tensor / row_max
    return normalized_tensor


def weighted_attention_maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, attention_masks, mask_matched_idxs):
    ...
    """
    # discretization_size = mask_logits.shape[-1]
    discretization_size_1 = mask_logits.shape[-2]
    discretization_size_2 = mask_logits.shape[-1]
    labels = [gt_label[idxs] for gt_label, idxs in zip(gt_labels, mask_matched_idxs)]

    mask_targets = [
        project_masks_on_boxes_with_dif_side(m, p, i, discretization_size_1, discretization_size_2) for m, p, i in zip(gt_masks, proposals, mask_matched_idxs)
    ]
    labels = torch.cat(labels, dim=0)
    mask_targets = torch.cat(mask_targets, dim=0)
    # torch.mean (in binary_cross_entropy_with_logits) doesn't
    ...
    mask_targets = normalize_rows(mask_targets)

    # Gaussian weighted
    kernel_size = 4
    min_threshold = 0.2
    expanded_gaussian_weights = F.avg_pool1d(mask_targets.transpose(1, 2), kernel_size, stride=1, padding=kernel_size // 2)
    expanded_gaussian_weights = expanded_gaussian_weights.transpose(1, 2)
    expanded_gaussian_weights = torch.maximum(expanded_gaussian_weights, torch.tensor(min_threshold, device=expanded_gaussian_weights.device))
    
    # crop gauss weight
    target_height, target_width = mask_targets[0].shape[-2], mask_targets[0].shape[-1]
    expanded_gaussian_weights = expanded_gaussian_weights[:, :target_height, :target_width]

    # Calculate mask loss with no reduction
    mask_logits = mask_logits[torch.arange(labels.shape[0], device=labels.device), labels]
    mask_loss = F.binary_cross_entropy_with_logits(
        mask_logits, mask_targets, reduction='none'
    )

    # Resize attention_masks to the same size as gt_masks
    gt_mask_size = (gt_masks[0].shape[-2], gt_masks[0].shape[-1])  
    if attention_masks[0].shape[-2:] != gt_mask_size:
        attention_masks = resize_attention_masks(attention_masks, gt_mask_size)
    attention_masks = [
        project_masks_on_boxes_with_dif_side(m, p, i, discretization_size_1, discretization_size_2) for m, p, i in zip(attention_masks, proposals, mask_matched_idxs)
    ]

    # Apply attention masks to the calculated loss
    attention_masks = torch.cat(attention_masks, dim=0)

    # Check if attention_masks is empty
    if attention_masks.numel() == 0 or attention_masks.sum() == 0:
        return mask_logits.sum() * 0  # Return zero loss in a consistent manner

    weighted_loss = mask_loss * attention_masks * expanded_gaussian_weights

    # Sum the weighted loss and normalize by the number of positive samples
    mask_loss = weighted_loss.sum() / (attention_masks * expanded_gaussian_weights).sum()
    return mask_loss


def attention_maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, attention_masks, mask_matched_idxs):
    # type: (Tensor, List[Tensor], List[Tensor], List[Tensor], List[Tensor], List[Tensor]) -> Tensor
    """
    Args:
        proposals (list[BoxList])
    ...
    attention_masks = resize_attention_masks(attention_masks, gt_mask_size)
    attention_masks = [
        project_masks_on_boxes(m, p, i, discretization_size) for m, p, i in zip(attention_masks, proposals, mask_matched_idxs)
    ]
    ...
    # Check if attention_masks is empty
    if attention_masks.numel() == 0:
        return mask_logits.sum() * 0  # Return zero loss in a consistent manner

    weighted_loss = mask_loss * attention_masks
    
    # Sum the weighted loss and normalize by the number of positive samples
    mask_loss = weighted_loss.sum() / attention_masks.sum()
    return mask_loss


def focal_attention_maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, attention_masks, mask_matched_idxs):
    # type: (Tensor, List[Tensor], List[Tensor], List[Tensor], List[Tensor], List[Tensor]) -> Tensor
    """
    Args:
        proposals (list[BoxList])
    ...
    pt = mask_probs * mask_targets + (1 - mask_probs) * (1 - mask_targets)
    focal_weight = alpha * (1 - pt) ** gamma

    mask_loss = focal_weight * bce_loss
...
def expand_gt_and_attention_masks(gt_masks, attention_masks, image_shapes):
    """
    Expand gt_masks and attention_masks to match the expanded image size for each image in the batch.
    :param gt_masks: List[Tensor], each Tensor of shape (H, W), representing ground truth masks.
    :param attention_masks: List[Tensor], each Tensor of shape (H, W), representing attention masks.
    :param image_shapes: List[Tuple[int, int]], expanded image dimensions for each image in the batch.
    :return: Expanded gt_masks and attention_masks.
    """
    expanded_gt_masks = []
    expanded_attention_masks = []

    # Iterate over each image in the batch
    for gt_mask, attention_mask, (H_exp, W_exp) in zip(gt_masks, attention_masks, image_shapes):
        # Get original sizes
        H_gt, W_gt = gt_mask.shape[-2:]

        # Compute required padding
        pad_height = max(0, H_exp - H_gt)
        pad_width = max(0, W_exp - W_gt)

        # Apply padding to gt_masks
        expanded_gt_mask = torch.nn.functional.pad(
            gt_mask,
            (0, pad_width, 0, pad_height),  # (left, right, top, bottom)
            mode="constant",
            value=0  # Fill with 0
        )
        expanded_gt_masks.append(expanded_gt_mask)

        # Apply padding to attention_masks
        expanded_attention_mask = torch.nn.functional.pad(
            attention_mask,
            (0, pad_width, 0, pad_height),  # (left, right, top, bottom)
            mode="constant",
            value=0  # Fill with 0
        )
        expanded_attention_masks.append(expanded_attention_mask)
    ...
    def forward(
        ...
        if self.has_mask():
            ...
            if self.training:
                ...
                gt_labels = [t["labels"] for t in targets]
                attention_masks = [t["attention_masks"] for t in targets]

                gt_masks, attention_masks = expand_gt_and_attention_masks(gt_masks, attention_masks, image_shapes)

                rcnn_loss_mask = weighted_attention_maskrcnn_loss(mask_logits, mask_proposals, gt_masks, gt_labels, attention_masks, pos_matched_idxs)
